...
-   Forked country-state-city and stripped out city.json and its consumers
...
It accepts a valid `CountryCode` eg: `'AS'` and returns _Country Details_
...
It accepts a valid `StateCode` eg: `'TG'` `CountryCode` eg: `'IN'` and returns _State Details_
