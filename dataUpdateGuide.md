...
# State data
1. To add a new state, add the state data to the `allStates.geo.json` and `allStates.lite.json` in the respective country folder. Also, you need to add a new folder with name in the format `<stateName>-<stateIsoCode>` in the country folder
...
# Country data
1. To add a new country, add the country data to the `allCountries.geo.json`, `allCountries.lite.json`, `allCountries.timezones.json`. Also, you need to add a new folder with a name using the format `<countryName>-<countryIsoCode>` in the country folder and then add states as mentioned above.
